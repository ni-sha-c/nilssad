...
module combustion
	...
	double precision, parameter :: xf = 0.d0
	double precision, parameter :: beta = 0.d0, c1 = 0.d0, c2 = 0.d0
...
subroutine dXdt(X,dXdt_res,r)
	...
	do i = 1, N, 1
		...
		dXdt_res(N+i) = -1.d0*i*pi*X(i) - zeta(i)*X(N+i) &
						- 2.d0*beta*(abs(1.d0/3.d0 + uf(X))**0.5d0 &
						  - (1.d0/3.d0)**0.5d0)* &
						 sin(i*pi*xf)	
	end do

	dXdt_res(2*N+1) = -sigma*X(2*N+1) + sigma*X(2*N+2)
	dXdt_res(2*N+2) = X(2*N+1)*(r + 28.d0 -X(2*N+3)) - X(2*N+2)
	dXdt_res(2*N+3) = X(2*N+1)*X(2*N+2) - b*X(2*N+3)  
